...
. Add a `removeTag(Tag)` method. The specified group will be removed from everyone in the address book.
+
****
* Hints
...
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`]. Loop through each person, and remove the `group` from each person.
